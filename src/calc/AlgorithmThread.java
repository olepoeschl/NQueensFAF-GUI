...
public class AlgorithmThread extends Thread implements Serializable {
	...
	private void SetQueen21(int ld, int rd, int col, int idx) {
		...
		if(idx > mark1) {
			while(free > 0) {
				bit = free & (-free);
				free -= bit;
				SetQueen22((ld|bit)<<hop1, (rd|bit)>>hop1, col|bit, idx+1);
			}
			return;
		}
		while(free > 0) {
	...
	private void SetQueen22(int ld, int rd, int col, int idx) {
		if(idx > max) {
			if((~(ld | rd | col) & boardIntegers[idx]) > 0)
				tempcounter++;
			return;
		...
		while(free > 0) {
			bit = free & (-free);
			free -= bit;
			SetQueen22((ld|bit)<<1, (rd|bit)>>1, col|bit, idx+1);
		}
	}
	...
	private void SetQueen31(int ld, int rd, int col, int idx) {
		...
		if(idx > mark1) {
			while(free > 0) {
				bit = free & (-free);
				free -= bit;
				SetQueen32((ld|bit)<<hop1, (rd|bit)>>hop1, col|bit, idx+1);
			}
			return;
		}
		while(free > 0) {
	...
	private void SetQueen32(int ld, int rd, int col, int idx) {
		int free = ~(ld | rd | col) & boardIntegers[idx];
		int bit;
		if(idx > mark2) {
			while(free > 0) {
				bit = free & (-free);
				free -= bit;
				SetQueen33((ld|bit)<<hop2, (rd|bit)>>hop2, col|bit, idx+1);
			}
			return;
		}
		while(free > 0) {
			bit = free & (-free);
			free -= bit;
			SetQueen32((ld|bit)<<1, (rd|bit)>>1, col|bit, idx+1);
		}
	}
		
	private void SetQueen33(int ld, int rd, int col, int idx) {
		if(idx > max) {
			if((~(ld | rd | col) & boardIntegers[idx]) > 0)
				tempcounter++;
			return;
		...
		while(free > 0) {
			bit = free & (-free);
			free -= bit;
			SetQueen33((ld|bit)<<1, (rd|bit)>>1, col|bit, idx+1);
		}
	}
